...
static __attribute__((nonnull, warn_unused_result))
bool
buf_LRU_block_remove_hashed_for_rw_cmd(
/*========================*/
	buf_page_t*	bpage,	/*!< in: block, must contain a file page and
				be in a state where it can be freed; there
				may or may not be a hash index to the page */
...
loop:
	...
	if (buf_pool->init_flush[BUF_FLUSH_LRU]
		...
		n_bg_wait++;
		//fprintf(stderr, "background wait\n");
		goto loop;
	}
...
#ifdef RW_CMD
  /* Call the RW routine */
	block = buf_flush_single_page_with_rw_cmd(buf_pool, space, offset);

	if (block) {
		buf_pool_mutex_enter(buf_pool);

		buf_LRU_block_remove_hashed_for_rw_cmd((buf_page_t*) block, FALSE); 
...
#ifdef RW_CMD
  if (((buf_block_t*) bpage)->use_rw_cmd || bpage->target_rw_cmd) {
    goto func_exit;
  }
#endif
				...
				compressed page of an uncompressed page */
{
	const buf_page_t*	hashed_bpage;
	buf_pool_t*		buf_pool = buf_pool_from_bpage(bpage);
	ulint					fold = buf_page_address_fold(bpage->space, bpage->offset);
	rw_lock_t*		hash_lock = buf_page_hash_lock_get(buf_pool, fold);
	ib_mutex_t*		block_mutex = buf_page_get_mutex(bpage);
	...
	((buf_block_t*) bpage)->use_rw_cmd = true;
  ...
  buf_pool->freed_page_clock += 1;
	...
	case BUF_BLOCK_FILE_PAGE:
		//buf_page_set_state(bpage, BUF_BLOCK_REMOVE_HASH);
		buf_page_set_state(bpage, BUF_BLOCK_READY_FOR_USE);
