...
#ifdef RW_CMD
	block = buf_LRU_get_free_block(buf_pool, 0, 0);
#else
...
#error "FIL_PAGE_LSN must be 64 bit aligned"
	...
	case SRV_CHECKSUM_ALGORITHM_STRICT_INNODB:
		...
		//fprintf(stderr, "case 4: checksum_field1: %u == %u ? checksum_field2: %u == %u == %u?\n",
			//checksum_field1, buf_calc_page_new_checksum(read_buf), checksum_field2, mach_read_from_4(read_buf + FIL_PAGE_LSN), buf_calc_page_old_checksum(read_buf));
		return(TRUE);
...
#ifdef RW_CMD
	/* Open the device */
	dev_fd = os_file_open_dev("/dev/nvme0n1");	
	fprintf(stderr, "Open a device for ioctl() %d\n", dev_fd);
#endif /* RW_CMD */
...
UNIV_INLINE
void
buf_block_init_low_with_rw_cmd(
/*===============*/
	buf_block_t*	block)	/*!< in: block to init */
{
	block->check_index_page_at_flush = FALSE;
...
#ifdef RW_CMD
	if (block != NULL) {
			if (((buf_page_t*) block)->target_rw_cmd || block->use_rw_cmd) {
				rw_lock_s_unlock(hash_lock);
				block = NULL;
...
#ifdef RW_CMD
				&& (!(((buf_page_t*) block)->target_rw_cmd) && !block->use_rw_cmd)
#endif /* RW_CMD */
		) {
			rw_lock_x_lock(hash_lock);
...
#ifdef RW_CMD
/********************************************************************//**
Inits a RW target page to the buffer buf_pool. */
static __attribute__((nonnull))
void
buf_page_init_with_rw_cmd(
/*==========*/
	buf_pool_t*	buf_pool,/*!< in/out: buffer pool */
	ulint		space,	/*!< in: space id */
	ulint		offset,	/*!< in: offset of the page within space
	...
	buf_block_t*	block)	/*!< in/out: block to init */
{
	...
	buf_block_init_low_with_rw_cmd(block);
...
#ifdef RW_CMD
    if (sync) {
      block = buf_LRU_get_free_block(buf_pool, space, offset);
    } else {
      block = buf_LRU_get_free_block(buf_pool, 0, 0);
    }
#else
...
#ifdef RW_CMD
		if (block->use_rw_cmd) {
			if (mach_read_from_4(block->frame + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID) == space
					&& mach_read_from_4(block->frame + FIL_PAGE_OFFSET) == offset) {
				buf_page_init_with_rw_cmd(buf_pool, space, offset, fold, zip_size, block);
			} else {
...
#ifdef RW_CMD
		if (block->use_rw_cmd) {
			/* when using RW cmd, move the block to the head */
...
#ifdef RW_CMD
	free_block = buf_LRU_get_free_block(buf_pool, 0, 0);
#else
	...
	FILE*		file)		/*!< in/out: buffer where to print */
{
	...
	/* For log */
	fprintf(file, "\n Free block %lu Clean block %lu BG wait %lu SPF %lu\n",
							n_free_block, n_clean_block, n_bg_wait, n_no_block);
	fprintf(file, "\nTotal writes: LRU %lu, flush list %lu, single page %lu, rw cmd %lu\n\nTotal reads: %lu\n",
							n_lru_batch, n_ckpt_batch, n_spf, n_rwcmd, n_total_reads);
	/* end */
}
