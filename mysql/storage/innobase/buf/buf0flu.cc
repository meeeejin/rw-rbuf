				...
				buf_page_in_file(bpage) and in the LRU list */
{
	...
	fprintf(stderr,
		...
		" in the LRU list! %u, %u\n",
		(ulong) buf_page_get_state(bpage), bpage->space, bpage->offset);
...
#ifdef RW_CMD
	if (((buf_block_t*) bpage)->use_rw_cmd
      || bpage->target_rw_cmd
				...
				to the page */
{
	...
	case SRV_CHECKSUM_ALGORITHM_STRICT_INNODB:
		...
		//fprintf(stderr, "new checksum = %u (%u, %u)\n", checksum, mach_read_from_4(page + FIL_PAGE_SPACE_ID), mach_read_from_4(page + FIL_PAGE_OFFSET));
		mach_write_to_4(page + FIL_PAGE_SPACE_OR_CHKSUM, checksum);
		checksum = (ib_uint32_t) buf_calc_page_old_checksum(page);
		//fprintf(stderr, "old checksum = %u (%u, %u)\n", checksum, mach_read_from_4(page + FIL_PAGE_SPACE_ID), mach_read_from_4(page + FIL_PAGE_OFFSET));
		break;
	...
	bool		sync)		/*!< in: true if sync IO request */
{
	...
	case BUF_BLOCK_FILE_PAGE:
		...
		/*fprintf(stderr, "org buf_flush_init %u, %u (%lu) with %lu %lu\n",
			bpage->space, bpage->offset, bpage->newest_modification,
			mach_read_from_4(((buf_block_t*) bpage)->frame + FIL_PAGE_LSN + 4), mach_read_from_4(((buf_block_t*) bpage)->frame + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4)); 
*/
		break;
	}
	...
	bool		sync)		/*!< in: true if sync IO request */
{
	...
	if (flush) {
		...
		/* For log */
		if (flush_type == BUF_FLUSH_LIST)	n_ckpt_batch++;
		if (flush_type == BUF_FLUSH_LRU)	n_lru_batch++;
		/* end */
...
#ifdef RW_CMD
/********************************************************************//**
Writes a page from the buffer pool to a file using RW command.
@return TRUE if the page was flushed */
UNIV_INTERN
bool
buf_flush_page_with_rw_cmd(
/*===========*/
	buf_pool_t*	buf_pool,	  /*!< in: buffer pool instance */
	buf_page_t*	bpage,		  /*!< in: buffer control block */
	buf_flush_t	flush_type,	/*!< in: type of flush */
	bool		    sync,		    /*!< in: true if sync IO request */
  ulint       space,      /*!< in: space id of the page to read */
  ulint       offset)     /*!< in: page number of the page to read */
{
	ib_mutex_t*	block_mutex = buf_page_get_mutex(bpage);
  bool        is_uncompressed;
	dberr_t			err;
	ibool       flush = FALSE;
	rw_lock_t*	rw_lock = &reinterpret_cast<buf_block_t*>(bpage)->lock;
	...
	if (flush) { 
		...
		err = fil_io_with_rw_cmd(OS_FILE_WRITE,
				sync, buf_page_get_space(bpage), zip_size,
				buf_page_get_page_no(bpage), 0,
				zip_size ? zip_size : UNIV_PAGE_SIZE,
				frame, bpage,
      ...
      flush = FALSE;
		...
		/* Instead of freeing the page, use the read page directly. */
		buf_page_io_complete(bpage);
...
#ifdef RW_CMD
		if (bpage->target_rw_cmd || ((buf_block_t*) bpage)->use_rw_cmd) {
			mutex_exit(block_mutex);
					...
					min_n) */
{
	...
	for (buf_page_t* bpage = UT_LIST_GET_LAST(buf_pool->flush_list);
	     ...
	     && bpage->oldest_modification < lsn_limit; ++scanned) {
	...
	buf_pool_t*	buf_pool)	/*!< in/out: buffer pool instance */
{
	...
	for (bpage = UT_LIST_GET_LAST(buf_pool->LRU), scanned = 1;
	     bpage != NULL; bpage = UT_LIST_GET_PREV(LRU, bpage), ++scanned) {
...
/******************************************************************//**
This function picks up a single dirty page from the tail of the LRU
list. Then, it calls a RW command to the storage with a page info to
be read. The RW command process the read operation first, then write
operation. It is called from user threads when they are unable to
find a replacable page at the tail of the LRU list. When the user
thread get a read ack, it moves the read page to the LRU header. 
@return TRUE if success. */
UNIV_INTERN
buf_block_t*
buf_flush_single_page_with_rw_cmd(
/*===========================*/
	buf_pool_t*	buf_pool,	/*!< in/out: buffer pool instance */
  ulint       space,    /*!< in: space id to read */
  ulint       offset)   /*!< in: page number to read */
{
	...
	/* Select a victim page to write */
	for (bpage = UT_LIST_GET_LAST(buf_pool->LRU);
	     bpage != NULL;
	     bpage = UT_LIST_GET_PREV(LRU, bpage)) {

		if (bpage->space < log_fd)	continue;

		ib_mutex_t*	block_mutex = buf_page_get_mutex(bpage);

		mutex_enter(block_mutex);
    
		if (bpage->buf_fix_count == 0 && buf_flush_ready_for_flush(bpage, BUF_FLUSH_SINGLE_PAGE)) {
      bpage->target_rw_cmd = true;

      flushed = buf_flush_page_with_rw_cmd(
				buf_pool, bpage, BUF_FLUSH_SINGLE_PAGE, true, space, offset);

			if (flushed) {
        break;
			}
      
      bpage->target_rw_cmd = false;
		}

		mutex_exit(block_mutex);
	}

	if (bpage == NULL || flushed == FALSE) {
		/* Can't find a single flushable page. */
		buf_pool_mutex_exit(buf_pool);
		return(NULL);
	}
