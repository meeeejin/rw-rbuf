	...
	mem_heap_t**		heap)	/*!< in/out: memory heap */
{
	...
	if (dict_table_is_comp(index->table)) {
		...
		case REC_STATUS_SUPREMUM:
			...
			break;
		default:
			fprintf(stderr, "error!! table id = %lu (%s) index name = %s with %d %d\n",
							index->table->id, index->table->name,	index->name, rec_get_status(rec),
							UNIV_EXPECT(rec_get_status(rec), REC_STATUS_ORDINARY));
			ut_error;
			return(NULL);
		}
	} else {
