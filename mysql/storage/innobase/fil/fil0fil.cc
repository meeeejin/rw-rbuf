...
#ifdef RW_CMD
	/* TODO: Generalize the path. Currently, only normal datafiles can use the RW command. */
	if (strcmp(node->name, "./tpcc10_4k/warehouse.ibd") == 0) {
		log_fd = space->id;
		fprintf(stderr, "setting log_fd %s to %lu\n", node->name, log_fd);
	}
#endif /* RW_CMD */

	/* For log */
	fprintf(stderr, "%s (space_id = %lu) with fd %d\n", node->name, space->id, node->handle);
...
retry:
	...
	if (space_id == 0 || space_id >= SRV_LOG_SPACE_FIRST_ID
      || space_id_to_read == 0 || space_id_to_read >= SRV_LOG_SPACE_FIRST_ID) {
		/* We keep log files and system tablespace files always open;
		this is important in preventing deadlocks in this module, as
		a page read completion often performs another read from the
		insert buffer. The insert buffer is in tablespace 0, and we
	...
	space = fil_space_get_by_id(space_id);
  space_to_read = fil_space_get_by_id(space_id_to_read);

	if (space != NULL && space->stop_ios
      && space_to_read != NULL && space_to_read->stop_ios) {
		/* We are going to do a rename file and want to stop new i/o's
		for a while */
	...
	if (!space || UT_LIST_GET_FIRST(space->chain)->open
	    || !space_to_read || UT_LIST_GET_FIRST(space_to_read->chain)->open) {
		return;
	}
...
#ifdef RW_CMD
/********************************************************************//**
Read and write data using the RW command. */
UNIV_INTERN
dberr_t
fil_io_with_rw_cmd(
/*===*/
	ulint	type,	
	bool	sync,		/*!< in: true if synchronous aio is desired */
	ulint	space_id,	/*!< in: space id of a page to write*/
	ulint	zip_size,	/*!< in: compressed page size in bytes;
				0 for uncompressed pages */
	ulint	block_offset,	/*!< in: offset in number of blocks of a page to write */
	ulint	byte_offset,	/*!< in: remainder of offset in bytes; in
				aio this must be divisible by the OS block
				size */
	ulint	len,		/*!< in: how many bytes to read or write; this
	...
	void*	message,	/*!< in: message for aio handler if non-sync
				aio used, else ignored */
  ulint space_id_to_read, /*!< in: space id of a page to read */
  ulint offset_to_read)   /*!< in: page number of a page to read */ 
{
	ulint		mode;
	ulint		wake_later;
	ibool		ignore_nonexistent_pages;
  ...
  fil_space_t*	space_w;
	fil_node_t*	node_w;
	os_offset_t	offset_w;
	
  fil_space_t*	space_r;
	fil_node_t*	node_r;
	os_offset_t	offset_r;
...
# error "(1 << UNIV_PAGE_SIZE_SHIFT_MIN) != UNIV_PAGE_SIZE_MIN"
  ...
  srv_stats.data_written.add(len);
	...
	/* Reserve the fil_system mutex and make sure that we can open at
	least one file while holding it, if the file is not already open */
	fil_mutex_enter_and_prepare_for_rw_io(space_id, space_id_to_read);

	space_w = fil_space_get_by_id(space_id);
	node_w = UT_LIST_GET_FIRST(space_w->chain);

  space_r = fil_space_get_by_id(space_id_to_read);
  node_r = UT_LIST_GET_FIRST(space_r->chain);
	...
	for (;;) {
		if (node_r == NULL) {
			...
			fil_report_invalid_page_access(
				offset_to_read, space_id_to_read, space_r->name,
				byte_offset, len, type);

			ut_error;

		} else if (fil_is_user_tablespace_id(space_r->id)
			   && node_r->size == 0) {

			/* We do not know the size of a single-table tablespace
			before we open the file */
			break;
		} else if (node_r->size > offset_to_read) {
			/* Found! */
			break;
		} else {
			offset_to_read -= node_r->size;
			node_r = UT_LIST_GET_NEXT(chain, node_r);
		}
	}
	...
	for (;;) {
		if (node_w == NULL) {
			...
			fil_report_invalid_page_access(
				block_offset, space_id, space_w->name,
				byte_offset, len, type);

			ut_error;

		} else if (fil_is_user_tablespace_id(space_w->id)
			   && node_w->size == 0) {

			/* We do not know the size of a single-table tablespace
			before we open the file */
			break;
		} else if (node_w->size > block_offset) {
			/* Found! */
			break;
		} else {
			block_offset -= node_w->size;
			node_w = UT_LIST_GET_NEXT(chain, node_w);
		}
	}

	/* Open file if closed */
	if (!fil_node_prepare_for_io(node_r, fil_system, space_r)) {
		if (space_r->purpose == FIL_TABLESPACE
		    && fil_is_user_tablespace_id(space_r->id)) {
			mutex_exit(&fil_system->mutex);
			...
			ib_logf(IB_LOG_LEVEL_ERROR,
				...
				"i/o type %lu, space id %lu, page no %lu, "
				"i/o length %lu bytes",
				(ulong) type, (ulong) space_id_to_read,
				(ulong) offset_to_read, (ulong) len);
  ...
  if (!fil_node_prepare_for_io(node_w, fil_system, space_w)) {
		if (space_w->purpose == FIL_TABLESPACE
		    && fil_is_user_tablespace_id(space_w->id)) {
			mutex_exit(&fil_system->mutex);
	...
	/* Check that at least the start offset is within the bounds of a
	single-table tablespace, including rollback tablespaces. */
	if (UNIV_UNLIKELY(node_r->size <= offset_to_read)
	    && space_r->id != 0 && space_r->purpose == FIL_TABLESPACE) {

		fil_report_invalid_page_access(
			offset_to_read, space_id_to_read, space_r->name, byte_offset,
			len, type);

		ut_error;
	}

	if (UNIV_UNLIKELY(node_w->size <= block_offset)
	    && space_w->id != 0 && space_w->purpose == FIL_TABLESPACE) {

		fil_report_invalid_page_access(
			block_offset, space_id, space_w->name, byte_offset,
			len, type);
	...
	/* Calculate the low 32 bits and the high 32 bits of the file offset */
	if (!zip_size) {
    offset_r = ((os_offset_t) offset_to_read << UNIV_PAGE_SIZE_SHIFT)
			+ byte_offset;
		
    offset_w = ((os_offset_t) block_offset << UNIV_PAGE_SIZE_SHIFT)
			+ byte_offset;

		ut_a(node_r->size - offset_to_read
		     >= ((byte_offset + len + (UNIV_PAGE_SIZE - 1))
			 / UNIV_PAGE_SIZE));
		
    ut_a(node_w->size - block_offset
		     >= ((byte_offset + len + (UNIV_PAGE_SIZE - 1))
			 / UNIV_PAGE_SIZE));
	} else {
		ulint	zip_size_shift;
		...
		offset_r = ((os_offset_t) offset_to_read << zip_size_shift)
			+ byte_offset;
		ut_a(node_r->size - offset_to_read
		     >= (len + (zip_size - 1)) / zip_size);
	
		offset_w = ((os_offset_t) block_offset << zip_size_shift)
			+ byte_offset;
		ut_a(node_w->size - block_offset
		     >= (len + (zip_size - 1)) / zip_size);
  }
	...
	ret_rw = os_rw_io(type, mode | wake_later, node_w->name, node_w->handle,
					buf, offset_w, len, node_r->name, node_r->handle, buf, offset_r, len); 	

	ut_a(ret_rw);

  mutex_enter(&fil_system->mutex);

  fil_node_complete_io(node_w, fil_system, type);
  fil_node_complete_io(node_r, fil_system, type);
