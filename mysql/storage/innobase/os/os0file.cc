...
#ifdef RW_CMD
/*******************************************************************//**
Requests an i/o operation using a RW command.
@return	TRUE if request was queued successfully, FALSE if fail */
UNIV_INTERN
ibool
os_rw_io_func(
	...
	ulint		n_r)	/*!< in: number of bytes to read */
{
  ...
  ut_a(type == OS_FILE_WRITE);
	...
	/* If off_t is > 4 bytes in size, then we assume we can pass a
	64-bit address */
	offs_w = (off_t) offset_w;
	offs_r = (off_t) offset_r;

	if (sizeof(off_t) <= 4) {
		if (offset_w != (os_offset_t) offs_w || offset_r != (os_offset_t) offs_r) {
			ib_logf(IB_LOG_LEVEL_ERROR,
				"File RW at offset > 4 GB");
		}
	}
	...
	MONITOR_ATOMIC_INC(MONITOR_OS_PENDING_WRITES);
 	...
 	if (n_w == UNIV_PAGE_SIZE) {
		int ioctl_ret;
		ib_uint64_t write_lba;
		ib_uint64_t read_lba;
		__u32 result;
		__u32 nlb_w = ((__u32) n_w) / 4096 -1;
		__u32 nlb_r = ((__u32) n_r) / 4096 -1;

		/* Get the LBA */
		write_lba = buf_get_lba(node_w, offs_w);
		read_lba = buf_get_lba(node_r, offs_r);
		//fprintf(stderr, "Get read LBA %lu / write LBA %lu, data_len = %lu\n", read_lba, write_lba, n_w); 
		...
		/* Read the data using ioctl */
		ioctl_ret = ioctl(dev_fd, NVME_IOCTL_IO_CMD, &rw_cmd);

		if (ioctl_ret == -1) {
			perror("RW: ioctl fails");
			ut_error;
		} else {
			result = rw_cmd.result;
		}
	} else {
    ut_error;
  }
