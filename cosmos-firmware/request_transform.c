...
void ReqTransNvmeToSlice(unsigned int cmdSlotTag, unsigned int startLba, unsigned int nlb, unsigned int cmdCode, unsigned int fua, unsigned int customCmd)
{
	...
	if(customCmd == REQ_OPT_CUSTOM_CMD_ON)
	{
		...
		xil_printf("CUSTOM CMD reqCode:%X cmdSlotTag:%d StartLba:%d NvmeBlock:%d\r\n", reqCode, cmdSlotTag, startLba, requestedNvmeBlock);
#endif
	}
	else
	{
#if (PRINT_DEBUG_MSG == 1)
		xil_printf("IO CMD reqCode:%X cmdSlotTag:%d StartLba:%d NvmeBlock:%d\r\n", reqCode, cmdSlotTag, startLba, requestedNvmeBlock);
#endif
	}
	...
	reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf = REQ_OPT_CUSTOM_CMD_BUF_OFF;
...
void EvictDataBufEntry(unsigned int originReqSlotTag)
{
	...
	if(dataBufMapPtr->dataBuf[dataBufEntry].dirty == DATA_BUF_DIRTY)
	{
		...
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf = reqPoolPtr->reqPool[originReqSlotTag].reqOpt.ccmdDataBuf;
...
void EvictCcmdDataBufEntry(unsigned int originReqSlotTag)
{
	...
	dataBufEntry = reqPoolPtr->reqPool[originReqSlotTag].dataBufInfo.entry;
	if(ccmdDataBufMapPtr->dataBuf[dataBufEntry].dirty == DATA_BUF_DIRTY)
	{
		reqSlotTag = GetFromFreeReqQ();
		virtualSliceAddr =  AddrTransWrite(ccmdDataBufMapPtr->dataBuf[dataBufEntry].logicalSliceAddr);

		reqPoolPtr->reqPool[reqSlotTag].reqType = REQ_TYPE_NAND;
		reqPoolPtr->reqPool[reqSlotTag].reqCode = REQ_CODE_WRITE;
		reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag = reqPoolPtr->reqPool[originReqSlotTag].nvmeCmdSlotTag;
		reqPoolPtr->reqPool[reqSlotTag].logicalSliceAddr = ccmdDataBufMapPtr->dataBuf[dataBufEntry].logicalSliceAddr;
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ENTRY;
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandAddr = REQ_OPT_NAND_ADDR_VSA;
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandEcc = REQ_OPT_NAND_ECC_ON;
		reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandEccWarning = REQ_OPT_NAND_ECC_WARNING_ON;
		...
		reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = dataBufEntry;
		UpdateCcmdDataBufEntryInfoBlockingReq(dataBufEntry, reqSlotTag);
		reqPoolPtr->reqPool[reqSlotTag].nandInfo.virtualSliceAddr = virtualSliceAddr;
		...
		ccmdDataBufMapPtr->dataBuf[dataBufEntry].dirty = DATA_BUF_CLEAN;
...
void CcmdDataReadFromNand(unsigned int originReqSlotTag)
{
	...
	if(virtualSliceAddr != VSA_FAIL)
	{
		...
		reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = reqPoolPtr->reqPool[originReqSlotTag].dataBufInfo.entry;
		UpdateCcmdDataBufEntryInfoBlockingReq(reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry, reqSlotTag);
		reqPoolPtr->reqPool[reqSlotTag].nandInfo.virtualSliceAddr = virtualSliceAddr;
...
void ReqTransSliceToLowLevel()
{
	...
	while(sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
	{
...
#if (PRINT_DEBUG_MSG == 1)
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
				xil_printf("Hit data buffer reqSlotTag:%d dataBufEntry:%d\r\n", reqSlotTag, dataBufEntry);
#endif
		}
		else
		{
			//allocate a data buffer entry for this request
			customBufEntry = CheckCcmdDataBufHit(reqSlotTag);

			if(customBufEntry != DATA_BUF_FAIL)
			{
				dataBufEntry = customBufEntry;
				//data buffer hit
				reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry = dataBufEntry;
				reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf = REQ_OPT_CUSTOM_CMD_BUF_ON;
#if (PRINT_DEBUG_MSG == 1)
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
					xil_printf("Hit custom data buffer reqSlotTag:%d dataBufEntry:%d\r\n", reqSlotTag, dataBufEntry);
#endif
			}
			else
			{
				...
				else //reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_WRITE
				{
					reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf = REQ_OPT_CUSTOM_CMD_BUF_OFF;
				}

				//data buffer miss, allocate a new buffer entry
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
					dataBufEntry = AllocateCcmdDataBuf();
				else
				...
				{
					if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
						xil_printf("Miss custom data buffer reqSlotTag:%d bufferEnt:%d\r\n", reqSlotTag, dataBufEntry);
					else
						xil_printf("Miss data buffer reqSlotTag:%d bufferEnt:%d\r\n", reqSlotTag, dataBufEntry);
				}
#endif

				//clear the allocated data buffer entry being used by a previous request
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
					EvictCcmdDataBufEntry(reqSlotTag);
				else
				...
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
				{
					ccmdDataBufMapPtr->dataBuf[dataBufEntry].logicalSliceAddr = reqPoolPtr->reqPool[reqSlotTag].logicalSliceAddr;
					PutToCcmdDataBufHashList(dataBufEntry);
				}
				else
				{
				...
				{
					if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
						CcmdDataReadFromNand(reqSlotTag);
					else
		...
		{
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
				ccmdDataBufMapPtr->dataBuf[dataBufEntry].dirty = DATA_BUF_DIRTY;
			else
		...
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
		{
			...
			if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_RxDMA)
			{
				...
				AllocateNotIssuedRxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, GenerateDataBufAddr(reqSlotTag));
			}
...
void ReleaseBlockedByBufDepReq(unsigned int reqSlotTag)
{
	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
		{
			if(ccmdDataBufMapPtr->dataBuf[reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry].blockingReqTail == reqSlotTag)
				ccmdDataBufMapPtr->dataBuf[reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry].blockingReqTail = REQ_SLOT_TAG_NONE;
		}
		else
		{
		...
		while(numOfNvmeBlock < reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock)
		{
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.customCmd == REQ_OPT_CUSTOM_CMD_ON)
			{
				...
				if(numOfNvmeBlock == reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock - 1)
					IssuedRxforCustomIoCmd(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, reqSlotTag, reqPoolPtr->reqPool[reqSlotTag].reqCode);
			}
			else
			{
