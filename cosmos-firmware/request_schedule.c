	...
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NAND)
		{
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
				return (CCMD_DATA_BUFFER_BASE_ADDR + reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry * BYTES_PER_DATA_REGION_OF_SLICE);
		...
		{
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
				return (CCMD_DATA_BUFFER_BASE_ADDR + reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry * BYTES_PER_DATA_REGION_OF_SLICE + reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.nvmeBlockOffset * BYTES_PER_NVME_BLOCK);
			else
...
unsigned int GenerateSpareDataBufAddr(unsigned int reqSlotTag)
{
	if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.ccmdDataBuf == REQ_OPT_CUSTOM_CMD_BUF_ON)
	{
		...
		{
			if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ENTRY)
				return (CCMD_SPARE_DATA_BUFFER_BASE_ADDR + reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.entry * BYTES_PER_SPARE_REGION_OF_SLICE);
			else if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_TEMP_ENTRY)
